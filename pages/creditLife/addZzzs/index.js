...
Page({
  ...
  onUpZzzs: function(e) {
    ...
    this.onUpLoad(9, fileName, "onUpZzzs", newFilesUids_str);
  },
  // 提交上传
  onSubmit: function() {
    ...
    app.ajax.req('credit_api_controller/addAcertificatesProve', data, 'GET', function(res) {
  ...
  onShow: function() {
    ...
    console.log("上传资质证书onShow时uploadList=" + JSON.stringify(uploadList));
    var currImgs_url = [];
    ...
    var data = {};
    data.token = app.globalData.token;
    app.ajax.req('credit_api_controller/acertificatesProve', data, 'GET', function(res) {
      ...
      if (parseInt(res.errorCode) == 200) {
        ...
        if (creditImage) {
          for (let i = 0; i < creditImage.length; i++) {
            oldFiles_arr.push(creditImage[i].acertificatesImage);
            creditImage[i].acertificatesCertification = that.switchText(creditImage[i].acertificatesCertification);
          }
        }
        that.setData({
          creditImage,
