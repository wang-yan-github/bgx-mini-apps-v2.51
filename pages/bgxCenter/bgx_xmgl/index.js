...
Page({
  ...
  tsYyXmShow: function(that, offset, p_name) {
    ...
    data.limit = 10;
    data.name = p_name;
    console.log("所传参数==" + JSON.stringify(data));
    app.ajax.req('workerproject_api_controller/workerBespeakList', data, 'POST', function(res) {
      console.log("项目管理列表=" + JSON.stringify(res));
      wx.hideLoading();
  ...
  opratorTap: function(e) {
    ...
    console.log("text==" + text + ",,index==" + index + ".,,pid=" + pid);
    if (text == "详情") {
      console.log("包工侠详情pid==" + pid);
      wx.navigateTo({
          ...
          success: function(res) {
            if (res.confirm) {
              ...
              if (laiyuan == "系统推送") {
                ...
                app.ajax.req('workerproject_api_controller/workerAndworOk',
                  data, 'GET',
                  function(res) {
                    wx.hideLoading();
                    if (parseInt(res.errorCode) === 200) {
      ...
      wx.navigateTo({
        url: '../bgx_upHetong/index?wpid=' + wpid + '&type_=1'
      })
    } else if (text == "修改") {
      //修改合同
      wx.navigateTo({
        // url: 'bgx_editHetong?wpid=' + wpid + '&bid=' + bid + '&type_=' + 1
        url: '../bgx_editHetong/index?wpid=' + wpid + '&bid=' + bid + '&type_=' + 1
      })
    } else if (text == "施工") {
      wx.showModal({
        title: '确认施工',
        content: '注意：请确认当前项目是否已经开始施工？',
        confirmText: '确认施工',
        cancelText: '返回',
        success: function(res) {
          if (res.confirm) {
